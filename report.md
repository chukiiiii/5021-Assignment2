...
The environment models the triangular board, legal-action mask, stochastic placement rule, and terminal win conditions. Key design choices:
...
For neural network agents, the board is encoded as a **3 × 12 × 12** tensor:
...
### 2.3 Stage 1: Expert Data Generation
...
Each recorded sample is a `(state, action_mask, expert_action)` triplet. The final dataset contains 5685 expert decisions from heuristic and MCTS-guided play.

### 2.4 Stage 2: Behavior Cloning / Supervised Fine-Tuning
...
The resulting policy is directly used as the initialization for PPO fine-tuning.

### 2.5 Stage 3: Conservative PPO Fine-Tuning
...
- **PPO update:** Clipped surrogate objective (ϵ = 0.2), return-minus-value advantage estimate, entropy bonus.
- **Conservative regularization:** Lower learning rate, fewer update epochs, and a reference-policy KL penalty to reduce catastrophic forgetting.
- **Training budget:** 120 training episodes per seed.
...
- **Reporting:** Results are summarized as win rate, mean reward, and average game length.

### 3.2 Baseline Methods

We compare our core method against the following baselines:

- **Random** — chooses uniformly from available actions. Serves as the lowest comparison point.
- **Tabular Q-Learning** — a table-based Q-learning agent using self-play and canonical state. The 96-cell board creates a large state space that a flat table cannot generalize across.
- **Heuristic** — a hand-designed scoring function prioritizing immediate wins, blocking opponent threats, creating promising lines, playing near friendly stones, and avoiding high-forfeit-risk cells. Not trained; serves as a strong baseline and expert data generator.
- **DQN** — a convolutional neural network (two Conv2D layers + 256-unit FC) outputting 96 Q-values. Trained via self-play with epsilon-greedy exploration, experience replay, and target network.
- **PPO from scratch** — an actor-critic network with the same encoder as DQN plus separate policy and value heads. Trained via self-play rollouts with clipped surrogate objective (ϵ = 0.2) and entropy bonus.
...
4. **Conservative PPO fine-tuning preserves and improves.** Using a lower learning rate, fewer update epochs, and reference-policy regularization helps prevent catastrophic forgetting of BC knowledge. The conservative variant (40% vs heuristic, 50% vs MCTS) significantly outperforms the normal fine-tune (23.3% vs heuristic, 25% vs MCTS).
...
Implementation and reproduction details are documented separately in the project README.
